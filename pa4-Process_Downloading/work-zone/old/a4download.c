// TODO: 9/3
// did all the testing, now just need to refactor code to be cleaner in main

// 9/4 FOLLOWUP:
// this version tries to create a separate function that mutates file_info via a pointer. it is the same behavior as other.c
// the current one in the directory above returns a pointer that file_info points to, which allows us to refactor all of the
// file parsing & information copying into a separate function, leaving only main to deal with downloading!
...
int parse_args(int argc, char* argv[])
{
	...
	{
		// TODO: fix when user inputs 0 or something less than 0
...
// Function parses and verifies program arguments for correct startup
// Returns the max-processing time as inputted by user, a default value if unspecified
void parse_file(char* input_file, FileEntry** file_info, int* n_lines)
{
...
//		file_info[*n_lines]->file_name = line_info.file_name;
//		file_info[*n_lines]->url = line_info.url;
//		file_info[*n_lines]->timeout = line_info.timeout;
//		file_info[*n_lines]->line_number = line_info.line_number;
		(*n_lines)++;
	}
...
int main(int argc, char* argv[])
{
	...
	parse_file(input_file, &file_info, &n_lines);
